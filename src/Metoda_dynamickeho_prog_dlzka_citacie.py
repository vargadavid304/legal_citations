...
def find_sequences(array1, array2):
    sequences_decisions = []
    current_sequence_decision = []
    sequences_laws = []
    current_sequence_law = []
    first = True
    for element1, element2 in zip(array1, array2):
        if first == True:
            current_sequence_decision = [element1]
            current_sequence_law = [element2]
            first = False
        if ((not current_sequence_decision or element1 == current_sequence_decision[-1] + 1) or element1 == current_sequence_decision[-1]) or ((not current_sequence_law or element1 == current_sequence_law[-1] + 1) or element1 == current_sequence_law[-1]):
            if element1 == current_sequence_decision[-1] + 1 and element2 == current_sequence_law[-1] + 1:
                current_sequence_decision.append(element1)
                current_sequence_law.append(element2)
            else:
                continue
        else:
            if (len(current_sequence_decision) > 1 and is_sequence(current_sequence_decision)) and (len(current_sequence_law) > 1 and is_sequence(current_sequence_law)):
                sequences_decisions.append(current_sequence_decision)
                sequences_laws.append(current_sequence_law)
            current_sequence_decision = [element1]
            current_sequence_law = [element2]
    return sequences_decisions, sequences_laws
...
def find_best_citation():
    ...
    array_of_results = []
    arr_zakony_text, search_text, arr_id, arr_version = get_laws_for_judge_decision(decisions_text, date_decisions, 70)
    remove_punctuation_and_short_words(search_text)
    lcs = []
    list_best_citations = []
    for zakon in arr_zakony_text:
        lcs.append(longest_common_subsequence(search_text, zakon)[0])
        quotes = longest_common_subsequence(search_text, zakon)[1]
    ...
    print(("Pre toto: ", 70, " rozhodnutie, boli najdene citovane zakony s id: ", id, " a vo versions: ", version, "."))
